...
with tab3:
        ...
        for i, n in enumerate(names):
            ...
            if s and s["kind"] == "toggle":
                feats[n] = 1.0 if col.toggle(label, value=s.get("default", False),
                                             key=f"y_{n}") else 0.0
            elif s and s["kind"] == "slider":
                feats[n] = float(col.slider(label, s["min"], s["max"],
                                            s.get("default", (s["min"] + s["max"]) / 2),
                                            s.get("step", 1.0), key=f"y_{n}"))
            else:
    ...
    if run:
        ...
        fn, sv, base = yield_pred.shap_contributions(booster, feats)

        # plain-English summary
        top_i = max(range(len(sv)), key=lambda i: abs(sv[i]))
        direction = "raised" if sv[top_i] >= 0 else "lowered"
        st.markdown(
            f"Starting from a baseline of **{base:.2f}**, this sample's features "
            f"adjust the predicted yield to **{pred:.2f}**. "
            f"The biggest single driver is **{fn[top_i].replace('_', ' ')}**, "
            f"which {direction} it by **{abs(sv[top_i]):.2f}**."
        )

        left, right = st.columns([1, 1.7], gap="large")
        with left:
            st.metric("Predicted yield", f"{pred:.3f}")
            st.caption(f"Baseline (avg prediction): {base:.3f}")
            eff = (pd.DataFrame({
                "Feature": [n.replace("_", " ") for n in fn],
                "Your value": [feats[n] for n in fn],
                "Effect on yield": sv,
            }).sort_values("Effect on yield", key=abs, ascending=False)
              .reset_index(drop=True))
            st.dataframe(
                eff.style.format({"Your value": "{:g}", "Effect on yield": "{:+.3f}"}),
                use_container_width=True, hide_index=True,
            )
        with right:
            st.markdown("**How this prediction was built**")
            st.caption("Each step starts at the baseline and adds one feature's effect — "
                       "green raises yield, red lowers it.")
            st.altair_chart(ui.waterfall(base, fn, sv, pred), use_container_width=True)

        with st.expander("Overall feature importance (across the whole model)"):
            st.caption("Unlike the waterfall above (which explains *this* prediction), "
                       "this shows which features matter most in general.")
            gn, gv = yield_pred.global_importance(booster)
            st.altair_chart(ui.importance_bar(gn, gv), use_container_width=True)
